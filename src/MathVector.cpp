...
void Vec3D::calcAbs(){
    float  result = std::sqrt( (this->X*this->X) + (this->Y * this->Y) + (this->Z * this->Z) );
    this->abs = result;
}
...
float Vec3D::dotProduct(Vec3D Mathvector) {
    float  result = this->X * Mathvector.X + this->Y * Mathvector.Y + this->Z * Mathvector.Z;
    return result;
}
...
Vec2D::Vec2D(float radius, float phi) : 
...
Vec3D::Vec3D(float radius, float phi, float theta) : CoordinateSystem3D(radius, phi, theta){
...
Vec3D::Vec3D(float radius, float phi, float height) : CoordinateSystem3D(radius, phi, height){
